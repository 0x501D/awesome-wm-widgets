...
local function worker(user_args)
    ...
    function(widget, stdout)
        ...
        for s in stdout:gmatch("[^\r\n]+") do
            local status, charge_str, _ = string.match(s, '.+: (.*), (%d?%d?%d)%%,?(.*)')
            if status ~= nil then
        ...
        elseif (charge >= 10 and charge < 20) then batteryType = "battery-caution%s-symbolic"
        elseif (charge >= 20 and charge < 50) then batteryType = "battery-low%s-symbolic"
        elseif (charge >= 50 and charge < 80) then batteryType = "battery-good%s-symbolic"
